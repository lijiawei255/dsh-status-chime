/**
 * dsh-voice-alerts — spoken status alerts for DeepSeek Harness.
 *
 * WHY THIS EXISTS
 ...
 *
 * SEVEN SCENES, WITH A DELIBERATE DURATION GRADIENT
 ...
 *
 *   `approval/request` (best effort) -> reuses the `needs-input` clip when the
 *     approval policy is `ask`. It is not delivered under the `never` policy.
 *
 * REPLAY SAFETY
 ...
 *   Verified by a real trigger (6 of 7):
...
/** Order used by the bare `/voice-alerts` command (short clips first). */
const PREVIEW_ORDER = [
  'turn-done', 'needs-input', 'goal-complete',
  'job-done', 'goal-blocked', 'job-failed', 'turn-error',
];
...
// ── asset resolution ──────────────────────────────────────────────────────
function clipCandidates(config, scene, extension) {
  const list = [];
  if (typeof config.clipsDir === 'string' && config.clipsDir) {
    list.push(join(config.clipsDir, `${scene}.${extension}`));
  }
  list.push(join(USER_DIR, 'clips', `${scene}.${extension}`));
  list.push(join(PACKAGE_ASSETS, 'clips', `${scene}.${extension}`));
  return list;
}

/** First existing candidate wins; null when the clip is nowhere to be found. */
function resolveClip(config, scene, extension) {
  for (const candidate of clipCandidates(config, scene, extension)) {
...
function spawnPlayer(ctx, config, scene, interrupt) {
  ...
  if (file === null) {
    warnOnce(
      ctx,
      `clip-missing:${scene}`,
      `no audio for ${scene}; looked in: ${clipCandidates(config, scene, extension).join(' | ')}`,
    );
    return null;
  }
...
function statusText(ctx, config) {
  ...
  return [
    ...
    `Usage: /${config.commandName ?? 'voice-alerts'} [on|off|status|test <scene>]`,
  ].join('\n');
}
  ...
  ctx.inject(['commands'], (commandsCtx) => {
    ...
    try {
      commandsCtx.effect(() => commandsCtx.commands.register({
        name: commandName,
        description: `Voice alert control: /${commandName} to preview all, on|off to toggle, test <scene>, status`,
        handler: (invocation) => {
          try {
            ...
            return Promise.resolve({
              kind: 'error',
              text: `Usage: /${commandName} (preview all) | /${commandName} on|off | /${commandName} status | /${commandName} test <scene>`,
            });
          } catch (error) {
...
export function apply(ctx) {
  ...
  try {
    const clips = JSON.parse(readFileSync(join(PACKAGE_ASSETS, 'clips.json'), 'utf8'));
