 ...
 *   for English run 1.92s -> 7.90s, monotonic in the same order as Chinese, with
 *   every adjacent pair separated by at least 22%.
          ...
          try {
            ...
            if (verb === 'lang' || verb === 'language') {
              ...
              if (!LANGUAGES.includes(requested)) {
                return Promise.resolve({
                  kind: 'error',
                  text: `Unknown language "${rest[0]}". Available: ${LANGUAGES.join(' / ')}`,
                });
              }
