...
function createDispatcher(ctx) {
  ...
  const flush = () => {
    ...
    if (spawnPlayer(ctx, config, chosen, config.interrupt !== false, resolved) !== null) {
      ctx.logger?.info?.(`[${name}] playing ${chosen} (${SCENES[chosen]?.label ?? ''}) [${resolved.file} via ${resolved.backend} from ${resolved.source}]`);
    }
  };
  ...
  return {
    ...
    playDirect(scene, interrupt = true) {
      ...
      if (child !== null) ctx.logger?.info?.(`[${name}] playing ${scene} (${SCENES[scene]?.label ?? ''}, manual) [${resolved.file} via ${resolved.backend} from ${resolved.source}]`);
      return child;
    },
    stop: stopPlayback,
  };
